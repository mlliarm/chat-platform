...
if __name__ == "__main__":
    ...
    # so it stays off unless explicitly asked for. Binding defaults to loopback
    # for the same reason: this app has no auth or rate limiting, and the API key
    # lives server-side, so anyone who can reach the port can spend the account's
    # credits. Override HOST only behind something that authenticates.
    app.run(
